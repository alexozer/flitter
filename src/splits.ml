...
let split_time (timer : Timer.t) ?now split_num =
  ...
  then Some Duration.zero
    ...
    | Paused (splits, start_time, pause_time) ->
      ...
      then None
      else if split_num = List.length splits
...
let updated_golds (timer : Timer.t) =
  ...
  | Timing (splits, _) | Paused (splits, _, _) | Done (splits, _) ->
    ...
    let new_durations =
      ...
      |> List.init ~f:(fun i ->
             if i >= List.length splits
             then List.nth old_durations i |> Option.join
             else
               Option.merge
...
let updated_pb (timer : Timer.t) =
  ...
  | Done (splits, _) ->
    Option.bind timer.pb ~f:(fun pb_run ->
        ...
        |> Option.bind ~f:(fun (new_t, old_t) ->
               if Time_ns.Span.(new_t < old_t)
               then Some (archive_done_run timer splits)
               else timer.pb ) )
;;
