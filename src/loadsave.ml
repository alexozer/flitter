...
module Game = struct
  ...
  let t_of_sexp sexp =
    ...
    if num_splits = 0 then of_sexp_error "No split names defined" sexp
      ...
      in
      if not history_runs_ok then
        of_sexp_error
          "Not all history runs have same number of splits as split_names" sexp
      else game
end
...
let load_golds (parsed_game : Game.t) =
  ...
  | Some segments ->
      List.map segments ~f:(fun seg ->
          {Gold.title= seg.title; duration= seg.duration} )
  | None ->
      List.map parsed_game.split_names ~f:(fun name ->
          {Gold.title= name; duration= None} )
