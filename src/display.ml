...
let center_pad width i =
  if I.width i > width then I.hcrop 0 (I.width i - width) i
  else
...
let time_status (timer : Timer.t) split_num =
  ...
  *)
  if Splits.is_gold timer split_num then Gold
  else
    ...
    | Some delta -> (
      ...
      | Some prev_delta ->
          if Time_ns.Span.(delta < zero) then
            if delta < prev_delta then Ahead_gain else Ahead_loss
          else if delta > prev_delta then Behind_loss
          else Behind_gain )
...
let show_delta (timer : Timer.t) split_num =
  ...
  *)
  ...
  | Timing (splits, _) | Paused (splits, _, _) | Done (splits, _) -> (
      if split_num < List.length splits then true
      else
        match time_status timer split_num with
        ...
        | Ahead_gain | Ahead_loss | Gold ->
            ...
            let gold =
              List.nth timer.golds split_num |> Option.bind ~f:Gold.duration
            in
            Option.map2 sgmt gold ~f:Time_ns.Span.( > )
...
let split_row (timer : Timer.t) width i =
  let bg_attr =
    ...
    | Timing (splits, _) | Paused (splits, _, _) ->
        if i = List.length splits then Colors.selection_bg
        else Colors.default_bg
  in
  ...
  let delta_image =
    if show_comparison then I.string uncolored_attr "-"
    else
      match Splits.ahead_by timer i with
      | None -> I.string uncolored_attr "-"
      | Some delta ->
          if not (show_delta timer i) then I.string uncolored_attr ""
          else
  ...
  let sgmt_image =
    let seg_time =
      if show_comparison then Splits.archived_segment_time timer i
  ...
  let time =
    if show_comparison then Splits.archived_split_time timer i
    else Splits.split_time timer i
  in
  let time_str =
    match time with Some t -> Duration.to_string t 1 | None -> "-"
  in
  ...
  let wr_image =
    Option.map timer.wr ~f:(fun wr_run ->
        ...
        in
        if i >= curr_split then default_img
        else
  ...
  let time_cols_combined =
    List.map time_cells ~f:(left_pad time_col_width) |> I.hcat
  in
...
let big_timer (timer : Timer.t) width =
  let time, color =
    ...
    | Done (splits, _) ->
        ...
        in
        Option.value_map timer.comparison ~default:(time, Colors.ahead_gain)
          ~f:(fun comp ->
            let comp_time =
              List.nth comp.splits last_split_num
              |> Option.bind ~f:Split.time
              |> Option.value_exn
                   ~message:"Last split of comparison found empty"
            in
            let color =
              if time < comp_time then Colors.rainbow ()
              else Colors.behind_loss
            in
            (time, color) )
  in
  ...
  let time_img =
    ...
    | Timing (splits, _) | Paused (splits, _, _) | Done (splits, _) -> (
        ...
        | Some pd, Some ppd ->
            ...
            let color =
              if Time_ns.Span.(diff < zero) then Colors.ahead_gain
              else Colors.behind_loss
            in
...
let best_possible_time (timer : Timer.t) width =
  let t =
    ...
    | Timing (splits, _) | Paused (splits, _, _) -> (
        ...
        let curr_gold =
          List.nth (Splits.updated_golds timer) curr_split
          |> Option.bind ~f:Gold.duration
        in
        let last_split_time =
          if curr_split = 0 then Some Duration.zero
        ...
        | ( Some future_sob'
          , Some curr_gold'
          , Some last_split_time'
          , Some curr_seg' ) ->
            Some
              Time_ns.Span.(
                last_split_time' + max curr_seg' curr_gold' + future_sob')
        | _ -> None )
    | Done (splits, _) ->
        List.nth splits (List.length splits - 1) |> Option.join
  in
  let time_img =
    match t with
    | Some t' ->
...
let rec subdivide_space color w h max_size =
  if w > max_size || h > max_size then
    let subspace =
      subdivide_space color ((w / 2) + 1) ((h / 2) + 1) max_size
    in
...
let display timer (w, h) =
  ...
  *)
  I.(
    preamble timer w <-> void w 1 <-> splits_header timer w <-> splits timer w
    <-> void w 1 <-> big_timer timer w <-> previous_segment timer w
    <-> sob timer w <-> best_possible_time timer w
